...
async def english_tts(file_name: str):
...
async def french_tts(file_name: str):
    API_URL = "https://api-inference.huggingface.co/models/BrunoHays/whisper-large-v3-french-illuin"
    headers = {"Authorization": f"Bearer {os.getenv('HUGGINGFACEHUB_API_TOKEN')}"}
    with open(file_name, "rb") as f:
        data = f.read()
    async with aiohttp.ClientSession() as session:
...
async def main():
    ...
    data = await english_tts("/home/isayahc/projects/FanTonic/Alina_Eng.wav")
    x=0
